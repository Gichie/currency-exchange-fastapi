...
async def test_get_currency_by_code(ac: AsyncClient, mock_currency_service: AsyncMock):
    ...
    mock_currency_service.get_currency_by_code.return_value = CurrencyBase(**CURRENCIES[0])
...
async def test_create_currency_success(ac: AsyncClient, mock_currency_service: AsyncMock):
    ...
    created_currency_model = CurrencyBase(id=99, **new_currency_data)
    ...
    expected_payload_object = CurrencyBase(**new_currency_data)
